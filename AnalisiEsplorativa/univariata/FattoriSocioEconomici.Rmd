...
p <- ggplot(displaced_data, aes(x = "", y = Count, fill = Displaced)) +
  ...
  geom_text(aes(label = Label), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values = c("#CC79A7", "#0072B2")) +  # Mantiene i colori originali
  theme(plot.title = element_text(hjust = 0.5, face="bold"))
...
# Determinare il valore massimo comune per l'asse y
...
p1 <- ggplot(mother_qual_data, aes(x = Mothers_qualification, y = Perc, fill = Mothers_qualification)) +
  ...
  geom_text(aes(label = paste0(round(Perc, 1), "%")), 
            ...
            vjust = -0.5,
            size = 3) + 
  ylim(0, max_y) +  # Imposta lo stesso limite massimo
  theme(plot.title = element_text(hjust = 0.5, face="bold"), 
...
# Determinare il valore massimo tra i due dataset
...
p2 <- ggplot(father_occ_data, aes(x = Fathers_occupation, y = Perc, fill = Fathers_occupation)) +
  ...
  theme_minimal() +
  geom_text(aes(label = paste0(round(Perc, 1), "%")), 
            vjust = -0.3, size = 2.5) +  # Testo più piccolo e posizionato sopra la barra
  ylim(0, max_y) +  # Imposta lo stesso limite massimo
  theme(plot.title = element_text(hjust = 0.5, face="bold"), 
        axis.text.x = element_text(angle = 45, hjust = 1)) # Ruota le categorie dell'asse X
...
if (!"GDP" %in% colnames(data)) {
  stop("La colonna 'GDP' non esiste nel dataset.")
}
...
if (!"Unemployment_rate" %in% colnames(data)) {
  stop("La colonna 'Unemployment_rate' non esiste nel dataset.")
}
...
# Controllo se la colonna esiste
if (!"Inflation_rate" %in% colnames(data)) {
  stop("La colonna 'Inflation_rate' non esiste nel dataset.")
}
