...
data$Application_order <- factor(data$Application_order, 
                                 ...
                                 labels = c("0", "1st", "2nd", "3rd", "4th", "5th", "6th", "9th"))

# Calcolo delle frequenze relative per Application Order
...
p <- ggplot(application_data, aes(x = Order, y = Perc, fill = Order)) +
  ...
  labs(title = "Barplot: Application Order", x = "Application Order", y = "Frequenza Relativa (%)") +
  theme_minimal() +
...
# Salvataggio del grafico in PDF
ggsave("Grafici/Barplot_Application_Order.pdf", plot = p, width = 10, height = 6)
```
...
hist_density_plot <- ggplot(data, aes(x = Previous_qualification_grade)) +
  geom_histogram(aes(y = ..density..), 
                 binwidth = diff(range(prev_qual_grade_data)) / 30,  
...
hist_density_plot <- ggplot(data, aes(x = Admission_grade)) +
  geom_histogram(aes(y = ..density..), 
                 binwidth = diff(range(admission_grade_data)) / 30,  
