...
# Creazione dei kernel density plot con asse y sincronizzato
density_plot <- ggplot(data_long, aes(x = Grade, fill = Target, y = ..density..)) +
  geom_density(alpha = 0.6, adjust = 1.2) +
  facet_grid(Target ~ Semester, scales = "fixed") +  # asse y sincronizzato per tutti i grafici
  scale_fill_manual(values = c("Graduate" = "#6495ED", "Enrolled" = "#32CD32", "Dropout" = "#FF6A6A")) +  
...
# Riorganizzazione dei dati in formato long
data_long <- data %>% 
  pivot_longer(cols = c(`Curricular_units_1st_sem_(grade)`, `Curricular_units_2nd_sem_(grade)`),
               names_to = "Semester", values_to = "Grade") %>%
  mutate(Semester = recode(Semester, 
                           "Curricular_units_1st_sem_(grade)" = "1st Semester", 
                           "Curricular_units_2nd_sem_(grade)" = "2nd Semester"))

# Creazione del violin plot con boxplot sovrapposto e outlier visibili
violin_box_plot <- ggplot(data_long, aes(x = Target, y = Grade, fill = Target)) +
  ...
  facet_wrap(~Semester, scales = "free_y") +
  scale_fill_manual(values = c("Graduate" = "#6495ED", "Enrolled" = "#32CD32", "Dropout" = "#FF6A6A")) +  
  labs(subtitle = "Violin and Boxplot", 
       x = "Target", 
       y = "Grade", 
       fill = "Target") +
  theme_minimal() +
...
# Combinazione dei grafici in un'unica figura
combined_plot <- violin_box_plot +
  plot_annotation(title = "Curricular Units Grade / Target") &
  theme(plot.title = element_text(hjust = 0.48, face = "bold", size = 14))

# Salvataggio del grafico in PDF
ggsave("Grafici/Combined_Violin_Boxplot_Curricular_Units_1st_2nd_Sem_Grade.pdf", plot = combined_plot, width = 16, height = 12)
