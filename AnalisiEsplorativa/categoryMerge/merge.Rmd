...
library(ggplot2)
library(dplyr)
library(gridExtra) # Per affiancare i grafici
...
✅1-Portuguese → Studenti portoghesi
2-Other Nationality → Tutti gli altri studenti
```{r}
# NATIONALITY
...
data <- data[order(data$Mothers_qualification), ] # Ordina i dati in base al codice numerico
...
# Calcolo delle frequenze relative per il dataset originale
freq_original <- table(data$Mothers_qualification)
original_data <- data.frame(Qualification = names(freq_original), 
                            Count = as.numeric(freq_original))
original_data$Perc <- original_data$Count / sum(original_data$Count) * 100

# Calcolo delle frequenze relative per il dataset con le categorie accorpate
freq_grouped <- table(data$Mothers_qualification_grouped)
grouped_data <- data.frame(Qualification = names(freq_grouped), 
                           Count = as.numeric(freq_grouped))
grouped_data$Perc <- grouped_data$Count / sum(grouped_data$Count) * 100

# Determinare il massimo valore di Percentuale per sincronizzare gli assi y
y_max <- max(original_data$Perc, grouped_data$Perc)

# Creazione del barplot originale con ordinamento basato sul codice
p1 <- ggplot(original_data, aes(x = factor(Qualification, levels = sort(as.numeric(Qualification))), y = Perc, fill = Qualification)) +
  geom_bar(stat = "identity", width = 0.7) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
        legend.position = "none") +  # Rimuove la legenda
  ylim(0, y_max)  # Sincronizzazione asse y
...
# Salvataggio del grafico in PDF
ggsave("Grafici/Barplot_Mothers_Qualification_Comparison.pdf", plot = final_plot, width = 24, height = 6)
...
data <- data[order(data$Fathers_qualification), ] # Ordina i dati in base al codice numerico
...
# Calcolo delle frequenze relative per il dataset originale
freq_original <- table(data$Fathers_qualification)
original_data <- data.frame(Qualification = names(freq_original), 
                            Count = as.numeric(freq_original))
original_data$Perc <- original_data$Count / sum(original_data$Count) * 100

# Calcolo delle frequenze relative per il dataset con le categorie accorpate
freq_grouped <- table(data$Fathers_qualification_grouped)
grouped_data <- data.frame(Qualification = names(freq_grouped), 
                           Count = as.numeric(freq_grouped))
grouped_data$Perc <- grouped_data$Count / sum(grouped_data$Count) * 100
...
# Salvataggio del grafico in PDF
ggsave("Grafici/Barplot_Fathers_Qualification_Comparison_Ordered.pdf", plot = final_plot, width = 24, height = 6)
...
# Creazione delle etichette personalizzate (1st, 2nd, 3rd, etc.)
...
# Mostrare il conteggio delle categorie corrette
...
```{r}
# COURSE - ACCORPAMENTO CON LABEL SU PIÙ RIGHE
...
p1 <- ggplot(original_data, aes(x = factor(Course, levels = sort(as.numeric(Course))), 
  ...
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
        legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylim(0, max_y)  # Sincronizzazione dell'asse y
...
```
```{r}
# APPLICATION MODE - NUOVO ACCORPAMENTO CON SEPARAZIONE "OLDER" E "SPECIAL DIPLOMAS"
...
# Creazione del barplot originale con asse Y sincronizzato
p1 <- ggplot(original_data, aes(x = factor(Application_mode, levels = sort(as.numeric(Application_mode))), 
  ...
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
        legend.position = "none") +
  ylim(0, y_max)  # Sincronizzazione dell'asse Y

# Creazione del barplot con le categorie accorpate con asse Y sincronizzato
...
```{r}
# FATHERS_OCCUPATION - ACCORPAMENTO 
...
# Calcolo delle frequenze relative per il dataset originale
freq_original <- table(data$Fathers_occupation)
original_data <- data.frame(Occupation = names(freq_original), 
                            Count = as.numeric(freq_original))
original_data$Perc <- original_data$Count / sum(original_data$Count) * 100

# Calcolo delle frequenze relative per il dataset con le categorie accorpate
freq_grouped <- table(data$Fathers_occupation_grouped)
grouped_data <- data.frame(Occupation = names(freq_grouped), 
                           Count = as.numeric(freq_grouped))
grouped_data$Perc <- grouped_data$Count / sum(grouped_data$Count) * 100

# Trova il massimo valore di frequenza relativa per sincronizzare l'asse y
max_y <- max(c(original_data$Perc, grouped_data$Perc))

# Creazione del barplot originale con asse y sincronizzato
p1 <- ggplot(original_data, aes(x = factor(Occupation, levels = sort(as.numeric(Occupation))), 
                                y = Perc, fill = Occupation)) +
  geom_bar(stat = "identity", width = 0.7) +
  labs(title = "Barplot: Father's Occupation (Originale)", 
       x = "Occupation", 
       y = "Frequenza Relativa (%)") +
  theme_minimal() +
  geom_text(aes(label = paste0(round(Perc, 1), "%")), vjust = -0.5, size = 4) +
  ylim(0, max_y) +  # Sincronizza l'asse y
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
        legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1)) # Ruota etichette nel grafico originale

# Creazione del barplot con le categorie accorpate con asse y sincronizzato
p2 <- ggplot(grouped_data, aes(x = reorder(Occupation, as.numeric(factor(Occupation, levels = levels(data$Fathers_occupation_grouped)))), 
                               y = Perc, fill = Occupation)) +
  geom_bar(stat = "identity", width = 0.7) +
...
# Salvataggio del grafico in PDF
ggsave("Grafici/Barplot_Fathers_Occupation_Comparison.pdf", 
...
```{r}
# MOTHERS_OCCUPATION - ACCORPAMENTO
...
# Calcolo delle frequenze relative per il dataset originale
freq_original <- table(data$Mothers_occupation)
original_data <- data.frame(Occupation = names(freq_original), 
                            Count = as.numeric(freq_original))
original_data$Perc <- original_data$Count / sum(original_data$Count) * 100

# Calcolo delle frequenze relative per il dataset con le categorie accorpate
freq_grouped <- table(data$Mothers_occupation_grouped)
grouped_data <- data.frame(Occupation = names(freq_grouped), 
                           Count = as.numeric(freq_grouped))
grouped_data$Perc <- grouped_data$Count / sum(grouped_data$Count) * 100
...
p1 <- ggplot(original_data, aes(x = factor(Occupation, levels = sort(as.numeric(Occupation))), 
                                y = Perc, fill = Occupation)) +
  geom_bar(stat = "identity", width = 0.7) +
  labs(title = "Barplot: Mother's Occupation (Originale)", 
       x = "Occupation", 
       y = "Frequenza Relativa (%)") +
  theme_minimal() +
  geom_text(aes(label = paste0(round(Perc, 1), "%")), vjust = -0.5, size = 4) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
        legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1)) # Ruota etichette per migliorare leggibilità

p2 <- ggplot(grouped_data, aes(x = Occupation, y = Perc, fill = Occupation)) +
  geom_bar(stat = "identity", width = 0.7) +
  ...
  geom_text(aes(label = paste0(round(Perc, 1), "%")), vjust = -0.5, size = 4) +
  ylim(0, max_y) +  # Sincronizza l'asse y
  scale_x_discrete(limits = levels(data$Mothers_occupation_grouped)) +  # Mantiene l'ordine corretto
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
        legend.position = "none")
...
# Salvataggio del grafico in PDF
ggsave("Grafici/Barplot_Mothers_Occupation_Comparison_Vertical.pdf", 
       plot = combined_plot, width = 16, height = 12)
...
print(data %>%
  filter(`Debtor` == 1 & `Tuition_fees_up_to_date` == 1 & `Target` != "Dropout"))
```
