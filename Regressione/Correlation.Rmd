...
# Convertiamo la matrice di correlazione in un formato lungo per ggplot2
melted_cor <- melt(cor_matrix)

# Creazione del grafico della matrice di correlazione con etichette numeriche
cor_plot <- ggplot(melted_cor, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile(color = "white") +  # Heatmap con bordi bianchi
  scale_fill_gradient2(low = "#FF6A6A", high = "#6495ED", mid = "white",
                       midpoint = 0, limit = c(-1,1), space = "Lab",
                       name = "Correlazione") +
  geom_text(aes(label = sprintf("%.2f", value)), color = "black", size = 3) +  # Etichette numeriche con 2 decimali
  theme_minimal() +
...
# Visualizzazione del grafico (opzionale)
print(cor_plot)
